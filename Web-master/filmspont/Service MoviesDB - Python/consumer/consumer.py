...
#conn.request("DELETE", "/rol/13", headers={'Content-type': 'application/json'})
...
create_movie_post = {
     ...
     'description': 'Niño busca piedra',
     'stars': '7',
     'year': '2001'
 }
json_data_post = json.dumps(create_movie_post)
conn.request("POST", "/movie", json_data_post, headers={'Content-type': 'application/json'})
#conn.request("GET", "/moviestars/1", headers={'Content-type': 'application/json'})
	


#conn.request("PUT", "/movie/1", json_data_post, headers={'Content-type': 'application/json'})
